...
"
      ...
      <el-form-item label="订单状态" prop="relevantStatu">
        <el-select v-model="queryParams.relevantStatu" placeholder="请选择订单状态" clearable size="small">
          <el-option
            v-for="dict in relevantStatuOptions"
            :key="dict.dictValue"
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:relevantorder:export']"
        >导出
        </el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="relevantorderList" @selection-change="handleSelectionChange">
...
" align="center" prop="email"/>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['system:relevantorder:edit']"
            v-if="scope.row.relevantStatu == 0"
          >审批
          </el-button>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="归口单位审核留言" prop="relevantMessage">
          <el-input v-model="form.relevantMessage" placeholder="请输入归口单位审核留言"/>
        </el-form-item>
      </el-form>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        this.loading = true;
        listRelevantorder(this.queryParams).then(response => {
          this.relevantorderList = response.rows;
        ...
        this.$refs["form"].validate(valid => {
          if (valid) {
            if (this.form.id != null) {
              this.form.relevantStatu='2';
              updateRelevantorder(this.form).then(response => {
                this.msgSuccess("修改成功, 订单未通过");
                this.open = false;
                this.getList();
              });
            } else {
              addRelevantorder(this.form).then(response => {
      ...
      handleUpdate(row) {
        ...
        getRelevantorder(id).then(response => {
      ...
      submitForm() {
        this.$refs["form"].validate(valid => {
          if (valid) {
            if (this.form.id != null) {
              this.form.relevantStatu='1';
              updateRelevantorder(this.form).then(response => {
                this.msgSuccess("修改成功, 订单已通过");
                this.open = false;
                this.getList();
              });
            } else {
      ...
      handleDelete(row) {
        ...
        this.$confirm('是否确认删除归口订单管理编号为"' + ids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function () {
          return delRelevantorder(ids);
        }).then(() => {
          this.getList();
      ...
      handleExport() {
        ...
        this.$confirm('是否确认导出所有归口订单管理数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function () {
          return exportRelevantorder(queryParams);
        }).then(response => {
